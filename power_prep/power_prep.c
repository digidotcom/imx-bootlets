...
#if defined(BOARD_CPX2)
	// MUX DUART to work with CPX2
	HW_PINCTRL_MUXSEL7_WR(0x3ffaff3f);
	
	printf("\r\nPowerPrep start initialize power for CPX2...\r\n");
...
void PowerPrep_SwitchVdddToDdcdcSource( void )
{
#if defined(BOARD_CPX2) || defined(BOARD_WR21)
	int reg;

	// Set VDD=1.5v = (0.8v + TRIG x 0.025v)
	HW_POWER_VDDDCTRL.B.TRG = 0x1E;
	// Disable the VDDD switching converter output
	HW_POWER_VDDDCTRL.B.DISABLE_FET= 0x0;

	// Enable 4P2 to control lin reg
	HW_POWER_DCDC4P2_WR(0x00800018);

	// Disable battery charger and enable 100 ohm load on 4P2
	HW_POWER_CHARGE_WR(0x00400000);

	// Set current limit on 4P2 lin reg to 10mA
	HW_POWER_5VCTRL.B.CHARGE_4P2_ILIMIT=0x1;

	// Power on 4P2 lin reg
	HW_POWER_5VCTRL.B.PWD_CHARGE_4P2=0x0;

	// Ramp current limit to charge caps see PMU app note AN4199
	// (400 mA, 200 mA, 100 mA, 50 mA, 20 mA, 10 mA) = (bit 5,bit 4, bit 3, bit 2, bit 1, bit 0)
	for( reg=1 ; reg < 64 /* 2^6 See CHARGE_4P2_ILIMIT */ ; reg++ ) {
	    HW_POWER_5VCTRL.B.CHARGE_4P2_ILIMIT=reg;
	    hw_digctl_MicrosecondWait(10);
	}

	// Ungate path from lin reg to DC-DC
	HW_POWER_DCDC4P2.B.ENABLE_DCDC=0x1;
	HW_POWER_5VCTRL.B.ENABLE_DCDC=0x1;
