...
/* Final VDDD settings will be setup in the boot_prep */
# define CONFIG_VDDA_TRG	1800	/* mV */
# define CONFIG_VDDA_BO		1700	/* 100 mV offset */
# define CONFIG_VDDIO_TRG	3300	/* mV */
# define CONFIG_VDDIO_BO	3150	/* 150 mV offset */
...
int _start( void )
{
	...
	// the ROM will set the mux itself, but until then, set the mux here.
	/* This also reconfigures PWM0 and PWM1 to work as GPIOs, (configured to DUART
	 * by rom code), so that they don't conflict with our DUART pins if a pull-up
	 * is added for working with I2C1.
	 */
...
// For EMI 200MHz,must enable SSP0 pin drive to 12mA,or the SD boot will fail.
	...
	*/
	ddi_power_SetVddio(CONFIG_VDDIO_TRG, CONFIG_VDDIO_BO);

#ifdef mx28
	/* We will set to the final VDDD voltage in the boot_prep,
...
int PowerPrep_ConfigurePowerSource( void )
{
	...
	*/
	...
	{
#ifdef ALWAYS_BOOT_FROM_5V_IF_DETECTED
		/* 5V detected. This boot option doesn't waste time looking for a good
		*  battery.  Battery powered operation and automatic voltage
		*  measurements are disabled.
		*/
		bBatteryReady = false;
		...
		printf("\r\n5V source detected. Booting from 5V. " \
			    "Battery powered operation disabled.\r\n");
		/* Disable automatic battery voltage measurements which seem unnecessary
		* for this configuration.
		*/
...
#if defined(NO_DCDC_BATT_SOURCE) || defined(ALWAYS_BOOT_FROM_5V_IF_DETECTED)
